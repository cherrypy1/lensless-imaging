...
from src.datasets.image_utils import image_to_numpy
    ...
    def log_images(self, batch):
        ...
        if "measurement" in batch:
            self.writer.add_image(
                "lensless",
                image_to_numpy(batch["measurement"][0]),
            )
        if "target_roi" in batch:
            self.writer.add_image(
                "target",
                image_to_numpy(batch["target_roi"][0]),
            )
        if "reconstruction_roi" in batch:
            self.writer.add_image(
                "reconstruction",
                image_to_numpy(batch["reconstruction_roi"][0]),
            )
